...
const Sidebar = ({ header, children, setActivatedSidebar, setOverlayActivated, selected }: Props) => {
    return (
        <div className={`fixed right-0 top-0 h-full w-full sm:w-2/3 md:w-1/2 xl:w-1/3 bg-background rounded-tl-2xl rounded-bl-2xl px-24 pt-24 pb-64 z-50
        transition-all duration-1000 ease-in-out ${selected ? '' : 'translate-x-full'}`}>
            <div className='flex justify-between items-center'>
                ...
                <CloseIcon onClick={() => {
                    if (setOverlayActivated) {
                        setOverlayActivated(false)
                    }
                    if (setActivatedSidebar) {
                        setActivatedSidebar('')
                    }
                }
