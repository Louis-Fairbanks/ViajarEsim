...
const Benefits = ({stepsToShow, showButton, showHeader = true} : Props) => {
    ...
    const translations = useTranslations('Benefits')
    const Beneficios = translations.raw('items')
    ...
    return (
        <div className="h-100 p-24 sm:p-64 space-y-24 sm:space-y-48 items-center text-center overflow-hidden">
            {showHeader && <SectionHeader title={translations('heading')} header={translations('subheading')} />}
            ...
            `} style={{ transform: `translateX(-${translateXPercentage}px)` }}>
                {Beneficios.map((benefit : any, index : number) => (
                   index < stepsToShow 
                   ? <AdvantageBlurb
                       key={index}
            ...
            {showButton && <Link href={`${cartItems.length > 0 ? '/pago' : '/destinos'}`}><ButtonDark extraClasses='px-48 mt-24 py-9'>{translations('quieroMieSIM')}</ButtonDark></Link>}
        </div>
    )
}
